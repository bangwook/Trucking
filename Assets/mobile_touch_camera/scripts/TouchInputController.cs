...
namespace BitBenderGames {
  ...
  public class TouchInputController : MonoBehaviour {
    ...
    private static List<RaycastResult> sListUIRaycast = new List<RaycastResult>();
    public List<LayerMask> maskList = new List<LayerMask>();
    ...
    public void AddMaskLayer(string str)
    {
      maskList.Add(LayerMask.NameToLayer(str));
    }
    ...
    private bool IsPointerOverUIObject() {
      ...
      var point = TouchWrapper.Touch0.Position;
      eventDataCurrentPosition.position = new Vector2(point.x, point.y);
 
      sListUIRaycast.Clear();
      EventSystem.current.RaycastAll(eventDataCurrentPosition, sListUIRaycast);
      
      foreach (var layerMask in maskList)
      {
        RaycastResult result = sListUIRaycast.Find(x => x.gameObject.layer == layerMask.value);
    ...
    public void Update() {
      ...
      if (TouchWrapper.IsFingerDown && !isDragging && IsPointerOverUIObject())
      {
        isInputOnLockedArea = true;
      }
