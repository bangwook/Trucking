		...
		override public string GetValue(string cell, string className, string sheetName, string columnName) {
			...
			} else if (array.Length == 6)
			{
				if(array[0].Length == 4) {
					...
					return StringUtilities.Format("new DateTime({0},{1},{2},{3},{4},{5})", array[0], array[1], array[2], array[3], array[4], array[5]);
				} else if(array[2].Length == 4) {
					//Format: DD/MM/YYYY/hh/mm/ss
					return StringUtilities.Format("new DateTime({2},{1},{0},{3},{4},{5})", array[0], array[1], array[2], array[3], array[4], array[5]);
				}
		...
		public override string FormatReload(int index, string cellVar, string className, string sheetName, string columnName) {
			...
			return StringUtilities.Format(@"
				...
				if( {2}.Trim().Length > 0 ) {
					...
					}else if(tmp{0}.Length == 6 && tmp{0}[0].ToString().Length == 4){
						_rows[i].{1} = new DateTime( tmp{0}[0], tmp{0}[1], tmp{0}[2], tmp{0}[3], tmp{0}[4], tmp{0}[5] );//DD/MM/YYYY/hh/mm/ss
					}else if(tmp{0}.Length == 6 && tmp{0}[2].ToString().Length == 4){
						_rows[i].{1} = new DateTime( tmp{0}[2], tmp{0}[1], tmp{0}[0], tmp{0}[3], tmp{0}[4], tmp{0}[5] );//DD/MM/YYYY/hh/mm/ss
					}else{
